...
static long ht_ctl_ioctl(struct file *file, unsigned int cmd, unsigned long __user arg)
{
	...
	case HT_IOC_FPS_STABILIZER_UPDATE:
	{
		...
		if (ht_online_config_buf[0] == '\0') {
			...
			char empty[] = "{}";
			if (copy_to_user((struct ht_fps_stabilizer_buf __user *) arg, &empty, strlen(empty))) { }
		} else {
			if (copy_to_user((struct ht_fps_stabilizer_buf __user *) arg, &ht_online_config_buf, PAGE_SIZE)) { }
		}
		break;
	}
